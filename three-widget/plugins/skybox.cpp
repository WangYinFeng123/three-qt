...
namespace  ThreeQt {
    ...
    void SkyBox::CreateSkyBox(string dirPath)
    {
        ...
        md->tw->initScene->dirtyBound();
        BoundingSphere bound = md->tw->initScene->getBound();
        Vec3 bc = bound.center();
        float br = bound.radius();

        osg::ref_ptr<osg::Drawable> drawable = new osg::ShapeDrawable(new osg::Sphere(bc,br * 10));
        ...
        skyNode->getStateSet()->setMode(GL_LIGHTING,osg::StateAttribute::OFF);
        skyNode->getStateSet()->setMode(GL_LIGHT1,osg::StateAttribute::OFF);
        skyNode->getStateSet()->setMode(GL_LIGHT0,osg::StateAttribute::OFF);
        md->tw->initScene->addChild(skyNode);
    }
}
