...
ref_ptr<Geode> makeAxis(float radious)
{
    ...
    StateSet* ss = geom->getOrCreateStateSet();
    BlendFunc* blendFunc = new osg::BlendFunc();
...
ref_ptr<Geode> makeGround(float radius) {
    ...
    Vec3 v000(Vec3(0,0,0) - Vec3(radius, 0.0f,radius));
    Vec3 vX(Vec3(radius, 0.0f,-radius));
    Vec3 vZ(Vec3(-radius, 0.0f,radius));

    Vec3 dx(Vec3(width / 100.0f,0.0f,0.0f));
    Vec3 dz(Vec3(0.0f,0.0f,height / 100.0f));

    DrawElementsUShort* primitives = new DrawElementsUShort(GL_LINES);

    Vec3Array* coords = new Vec3Array;
    Vec4Array* colors = new Vec4Array();
    int i = -1;
    for(int iz = 0;iz < dz.z() + 1;++iz)
    {
        for(int ix = 0;ix < dx.x() + 1;++ix)
        {
            coords->push_back(v000 + dx * ix);
            coords->push_back(vZ + dx * ix);
        ...
        coords->push_back(v000 + dz * iz);
        coords->push_back(vX + dz * iz);
        primitives->push_back(++i);
        primitives->push_back(++i);
        colors->push_back(Vec4(1,1,1,0.2));
        colors->push_back(Vec4(1,1,1,0.2));
...
ref_ptr<osg::Node> makeBase(float radius)
{
    ...
    Vec3 v000(Vec3(0,0,0) - Vec3(width*0.5f,0.0f,height*0.5f));
    Vec3 dx(Vec3(width / ((float)numTilesX),0.0,0.0f));
    Vec3 dz(Vec3(0.0f,0.0f,height / ((float)numTilesZ)));
