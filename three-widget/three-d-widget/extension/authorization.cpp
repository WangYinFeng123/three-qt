...
bool Authorization::verity() {
    QString authorizationFilePath = ThreeDExtension::settings.value("authorizationFilePath").toString();
    ...
    ThreeDExtension::settings.setValue("flag",flag);
    ...
    std::cout << "[Authorization] 授权截止时间：" << QDateTime::fromTime_t(deadline).toString("yyyy/M/d HH:mm").toLocal8Bit().data() << std::endl;
